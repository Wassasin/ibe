...
//! Source: https://link.springer.com/content/pdf/10.1007/978-3-662-46803-6_20.pdf
...
// Derived sizes
const PK_BYTES: usize = 7 * G1_BYTES + GT_BYTES;
const SK_BYTES: usize = 13 * SCALAR_BYTES;
const USK_BYTES: usize = 5 * G2_BYTES;
...
impl UserSecretKey {
    pub fn to_bytes(&self) -> [u8; USK_BYTES] {
        ...
        *kprime = self.kprime.to_compressed();
    ...
    pub fn from_bytes(bytes: &[u8; USK_BYTES]) -> CtOption<Self> {
        ...
        let kprime = G2Affine::from_compressed(kprime);

        d00.and_then(|d00| {
            d01.and_then(|d01| {
                d10.and_then(|d10| {
                    d11.and_then(|d11| {
                        kprime.map(|kprime| UserSecretKey {
                            d0: [d00, d01],
                            d1: [d10, d11],
                            kprime,
                        })
