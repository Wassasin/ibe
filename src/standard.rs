...
pub struct Parameters([G1Affine; CHUNKS]);
...
pub fn setup<R: Rng>(rng: &mut R) -> (PublicKey, SecretKey) {
    ...
    let g2prime: G1Affine = (g2 * alpha).into();
...
fn entangle(pk: &PublicKey, v: &Identity) -> G1Projective {
    ...
    for (ui, vi) in pk.u.0.iter().zip(bits(&v.0)) {
        ucoll = G1Projective::conditional_select(&(ui + ucoll).into(), &ucoll, vi);
    }
    ucoll
}
...
pub fn encrypt<R: Rng>(pk: &PublicKey, v: &Identity, m: &Message, rng: &mut R) -> CipherText {
    ...
    let c1 = bls12_381::pairing(&pk.g2, &pk.g1) * t + m.0;
    let c2 = (pk.g * t).into();
...
impl PublicKey {
    pub fn to_bytes(&self) -> [u8; PUBLICKEYSIZE] {
        ...
        let (g, g1, g2, uprime, u) = mut_array_refs![&mut res, 96, 96, 48, 48, PARAMETERSIZE];
    ...
    pub fn from_bytes(bytes: &[u8; PUBLICKEYSIZE]) -> CtOption<Self> {
        let (g, g1, g2, uprime, u) = array_refs![bytes, 96, 96, 48, 48, PARAMETERSIZE];

        let g = G2Affine::from_compressed(g);
        let g1 = G2Affine::from_compressed(g1);
        let g2 = G1Affine::from_compressed(g2);
...
impl SecretKey {
    ...
    pub fn from_bytes(bytes: &[u8; 48]) -> CtOption<Self> {
        G1Affine::from_compressed(bytes).map(|g2prime| SecretKey { g2prime })
    }
}
...
impl CipherText {
    pub fn to_bytes(&self) -> [u8; 720] {
        let mut res = [0u8; 720];
        let (c1, c2, c3) = mut_array_refs![&mut res, 576, 96, 48];
    ...
    pub fn from_bytes(bytes: &[u8; 720]) -> CtOption<Self> {
        let (c1, c2, c3) = array_refs![bytes, 576, 96, 48];
